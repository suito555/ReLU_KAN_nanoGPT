...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.kan=="efficient_KAN":
            print("using efficient_KAN for attention projection")
            self.c_attn = EfficientKANLinear(config.n_embd, 3 * config.n_embd)
            ...
            raise ValueError("kan name error")
        

        # output projection
        if config.kan=="efficient_KAN":
            print("using efficient_KAN for output projection")
            self.c_proj = EfficientKANLinear(config.n_embd, config.n_embd)
        elif config.kan=="ReLU_KAN":
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        if config.kan=="efficient_KAN":
            print("using efficient_KAN for MLP")
            self.c_fc    = EfficientKANLinear(config.n_embd, 4 * config.n_embd)
            self.c_proj  = EfficientKANLinear(4 * config.n_embd, config.n_embd)
        elif config.kan=="ReLU_KAN":
